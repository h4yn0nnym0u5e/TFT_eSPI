...
#define TFT_READ_BACKGROUND 0x00FFFFFF // special "background colour", means read from sprite or screen
...
// init() and begin() are equivalent, begin() included for backwards compatibility
  ...
  uint16_t drawPixel(int32_t x, int32_t y, uint32_t color, uint8_t alpha, uint32_t bg_color = TFT_READ_BACKGROUND);
  ...
  void     drawSmoothArc(int32_t x, int32_t y, int32_t r, int32_t ir, float startAngle, float endAngle, uint32_t fg_color, uint32_t bg_color, bool roundEnds = false);
  ...
  void     drawArc(int32_t x, int32_t y, int32_t r, int32_t ir, float startAngle, float endAngle, uint32_t fg_color, uint32_t bg_color, bool smoothArc = true);
  ...
  void     fillSmoothCircle(int32_t x, int32_t y, int32_t r, uint32_t color, uint32_t bg_color = TFT_READ_BACKGROUND);

           // Draw a rounded rectangle that has a line thickness of r-ir+1 and bounding box defined by x,y and w,h
           // The outer corner radius is r, inner corner radius is ir
           // The inside and outside of the border are anti-aliased
  void     drawSmoothRoundRect(int32_t x, int32_t y, int32_t r, int32_t ir, int32_t w, int32_t h, uint32_t fg_color, uint32_t bg_color = TFT_READ_BACKGROUND, uint8_t quadrants = 0xF);

           // Draw a filled rounded rectangle , corner radius r and bounding box defined by x,y and w,h
  void     fillSmoothRoundRect(int32_t x, int32_t y, int32_t w, int32_t h, int32_t radius, uint32_t color, uint32_t bg_color = TFT_READ_BACKGROUND);

           // Draw a small anti-aliased filled circle at ax,ay with radius r (uses drawWideLine)
           // If bg_color is not included the background pixel colour will be read from TFT or sprite
  void     drawSpot(float ax, float ay, float r, uint32_t fg_color, uint32_t bg_color = TFT_READ_BACKGROUND);

           // Draw an anti-aliased wide line from ax,ay to bx,by width wd with radiused ends (radius is wd/2)
           // If bg_color is not included the background pixel colour will be read from TFT or sprite
  void     drawWideLine(float ax, float ay, float bx, float by, float wd, uint32_t fg_color, uint32_t bg_color = TFT_READ_BACKGROUND);

           // Draw an anti-aliased wide line from ax,ay to bx,by with different width at each end aw, bw and with radiused ends
           // If bg_color is not included the background pixel colour will be read from TFT or sprite
  void     drawWedgeLine(float ax, float ay, float bx, float by, float aw, float bw, uint32_t fg_color, uint32_t bg_color = TFT_READ_BACKGROUND);
